 ...
 */
static JSBool
js_textDialogKbrd(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
  const char *message;
  char *input;
  JSBool ok, cancel;
  ...
  r = text_dialog_kbrd(message, &input, 
		    (ok     ? MESSAGE_POPUP_OK : 0) |
		    (cancel ? MESSAGE_POPUP_CANCEL : 0) | 
		    MESSAGE_POPUP_RICH_TEXT);
...
static JSFunctionSpec showtime_functions[] = {
    ...
    JS_FS("textDialogKbrd",       js_textDialogKbrd, 3, 0, 0),
    JS_FS_END
};
